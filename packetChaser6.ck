...
{      
    ...
    Math.sqrt(Math.pow(xvel[count],2) 
            + Math.pow(yvel[count],2)) => totalspeed;
    ...
    // Add killzone static
    if (xori[count] > -737 && xori[count] < -337 &&
        yori[count] > 1837 && yori[count] < 2237) {
        
        // Find x & y distances away from killzone
        xori[count] - xStatic => xDist; 
        yori[count] - yStatic => yDist;
            
        // Find linear distance from killzone
        Math.sqrt(Math.pow(xDist,2) 
            + Math.pow(yDist,2)) => totalDist;
            
          <<<totalDist>>>;
            
        // Adjust noise gain
        .1 * (1 + (-1 * totalDist / 300)) => n.gain;
    ...
    // Change gain based off velocity
